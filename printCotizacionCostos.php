...
$query_rsCotizacion = sprintf("SELECT * FROM subcotizacion,cotizacion WHERE subcotizacion.idcotizacion=cotizacion.idcotizacion AND idsubcotizacion = %s", GetSQLValueString($colname_rsCotizacion, "int"));
...
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_rsDetalle = sprintf("SELECT * FROM subcotizacionarticulo sb, articulo a WHERE idsubcotizacion = %s AND a.idarticulo = sb.idarticulo AND a.moneda=0", GetSQLValueString($colname_rsDetalle, "int"));
...
$colname_rsDetalle2= "-1";
if (isset($_GET['idsubcotizacion'])) {
  $colname_rsDetalle2 = $_GET['idsubcotizacion'];
}
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_rsDetalle2 = sprintf("SELECT * FROM subcotizacionarticulo sb, articulo a WHERE idsubcotizacion = %s AND a.idarticulo = sb.idarticulo AND a.moneda=1", GetSQLValueString($colname_rsDetalle2, "int"));
$rsDetalle2 = mysql_query($query_rsDetalle2, $tecnocomm) or die(mysql_error());
$row_rsDetalle2 = mysql_fetch_assoc($rsDetalle2);
$totalRows_rsDetalle2 = mysql_num_rows($rsDetalle2);
...
if($row_rsCotizacion['tipo']  == 0)
	$p = ($row_rsDetalle['precio_cotizacion'] ) ;
else{	
	$p = ($row_rsDetalle['precio_cotizacion'] ) ;
}
...
switch($paginas){
	...
	$pdf->AddPage();
	$pdf->createFondo();
	$pdf->setCostos("HOJA DE COSTOS HOJA DE COSTOS HOJA DE COSTOS");
	$pdf->setIP($row_rsCotizacion['idip']);
...
$pdf->setFirma($fir);
			...
			//$pdf->Output($row_rsCotizacion['identificador2'].'-COSTOS.pdf','I');
}

}//find e impresion en pesos

//costos en dolares

if($totalRows_rsDetalle2 > 0){
$sub=0;
$signo = array("$ ","US$ ");
$counter = 0;
$par = 0;
...
$counter++;
$par++;
 $long = strlen(trim($row_rsDetalle2['descri']));

if($long < 56){
$partida[] = $par;
$codigo[] =$row_rsDetalle2['codigo'];
$marca[] =$row_rsDetalle2['marca'];
$descripcion[] = utf8_decode(trim($row_rsDetalle2['descri']));
$cantidad[] = $row_rsDetalle2['cantidad'];
$medida[] =$row_rsDetalle2['medida'];

if($row_rsCotizacion['tipo']  == 0)
	$p = ($row_rsDetalle2['precio_cotizacion'] ) ;
else{	
	$p = ($row_rsDetalle2['precio_cotizacion'] ) ;
}
$precio[] = money_format('%!n', $p);
$importe2[] =money_format('%!n',($row_rsDetalle2['cantidad'] * $p));
$importe[] = money_format('%!n',(($row_rsDetalle2['cantidad'] * $p)));
$sub = $sub + money_format('%!n',($row_rsDetalle2['cantidad'] * $p));
	...
	$palabras = split(" ",utf8_decode(trim($row_rsDetalle2['descri'])));
...
}while($row_rsDetalle2 = mysql_fetch_assoc($rsDetalle2));
