...
$query_rsConversaciones = sprintf("SELECT DISTINCT c.*, u.nombrereal As nombreremitente FROM conversacion c JOIN conversacion_destinatario cd ON c.idconversacion = cd.idconversacion JOIN usuarios u ON u.id = c.remitente WHERE (c.remitente = %s OR cd.destinatario = %s) %s %s", GetSQLValueString($colname1_rsConversaciones, "int"),GetSQLValueString($colname2_rsConversaciones, "int"),$bus,$estado);
