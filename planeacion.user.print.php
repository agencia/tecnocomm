...
//echo $query_rsTareas;
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_rsUusarios = sprintf("SELECT * FROM usuarios where id=%s",GetSQLValueString($_GET['idusuario'], "int"));
...
$nombreUser=$row_rsUusarios['nombrereal'];
...
$pdf->Ln();
$pdf->Cell(25,5,'Responsable',1,0,'R');
$pdf->Cell(155,5,$nombreUser,1,0,'L');
////////////////////////////cotizaciones
$pdf->Ln();
$pdf->Ln();
...
$pdf->SetFont('Arial','B',8);
$pdf->SetTextColor(0);
$pdf->SetWidths(array(10,50,173,20));
$pdf->Row(array('IP','Consecutivo','Descripcion','Estado'));
//echo "HOLA";
//print_r($usuarios);
...
if(is_array($cotizaciones[$kusuario])){
	...
	foreach($cotizaciones[$kusuario] as $cotizacion){
		...
		$pdf->Row(array($coti[$cotizacion['idcotizacion']]['idip'],$coti[$cotizacion['idcotizacion']]['identificador2'],$coti[$cotizacion['idcotizacion']]['nombre'],$edotar[$cotizacion['estado']]));
...
if(is_array($levantamientos[$kusuario])){
	...
	foreach($levantamientos[$kusuario] as $levantamiento){
		...
		$pdf->Row(array($lev[$levantamiento['idlevantamiento']]['idip'],$lev[$levantamiento['idlevantamiento']]['consecutivo'],$lev[$levantamiento['idlevantamiento']]['descripcion'],$edotar[$levantamiento['estado']]));
...
if(is_array($ordenservicio[$kusuario])){
	...
	foreach($ordenservicio[$kusuario] as $orden){
		...
		$pdf->Row(array($ord[$orden['idordenservicio']]['idip'],$ord[$orden['idordenservicio']]['identificador'],$ord[$orden['idordenservicio']]['descripcionreporte'],$edotar[$orden['estado']]));
...
$pdf->SetFont('Arial','B',8);
$pdf->SetTextColor(0);
$pdf->SetWidths(array(233,25,20,1,1));
$pdf->Row(array('Descripcion', 'Estado'));
foreach($usuarios as $kusuario => $usuario){
//echo $kusuario."<br>";
if(is_array($administrativos[$kusuario])){
	...
	foreach($administrativos[$kusuario] as $administrativo){
		...
		$pdf->Row(array($administrativo['administrativo'],$edotar[$administrativo['estado']]));
...
$pdf->SetTextColor(0);
$pdf->SetWidths(array(10,50,173,20));
$pdf->Row(array('IP','Numero','Cliente','Estado'));
foreach($usuarios as $kusuario => $usuario){
//echo $kusuario."<br>";
if(is_array($facturas[$kusuario])){
	...
	foreach($facturas[$kusuario] as $factura){
		...
		$pdf->Row(array($factura['idip'],$facs[$factura['idfactura']]['numfactura'],$facs[$factura['idfactura']]['abreviacion'],$edotar[$factura['estado']]));
...
$pdf->Row(array('Numero','Proveedor','Monto','Estado'));
foreach($usuarios as $kusuario => $usuario){
//echo $kusuario."<br>";
if(is_array($cuentas[$kusuario])){
	...
	foreach($cuentas[$kusuario] as $cuenta){
		...
		$pdf->Row(array($cxp[$cuenta['idcuentaporpagar']]['nofactura'],$cxp[$cuenta['idcuentaporpagar']]['nameprov'],$cxp[$cuenta['idcuentaporpagar']]['monto'],$edotar[$cuenta['estado']]));
