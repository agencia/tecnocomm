...
class Conversacion extends FPDF{
	...
	function addConv($remitente=NULL,$destinatarios=array(),$mensaje=NULL,$estado=NULL,$prioridad=NULL,$fecha=NULL,$asunto=NULL,$folio = NULL){
		
		$this->Cell(30,5,"Remitente: ","TL",0,"L");
		$this->Cell(90,5,$remitente,"TLR",0,"L");
		$this->Cell(15,5,"Folio: ","TL",0,"L");
		$this->Cell(15,5,"M".$folio,"TLR",0,"L");
		$this->Ln();
		$this->Cell(30,5,"Asunto: ","TLB",0,"L");
		$this->MultiCell(120,5,$asunto,"TLRB","L");
...
$query_rsConversaciones = sprintf("SELECT DISTINCT c.*, u.nombrereal As nombreremitente FROM conversacion c JOIN conversacion_destinatario cd ON c.idconversacion = cd.idconversacion JOIN usuarios u ON u.id = c.remitente WHERE (c.remitente = %s OR cd.destinatario = %s) %s %s %s", GetSQLValueString($colname1_rsConversaciones, "int"),GetSQLValueString($colname2_rsConversaciones, "int"),$bus,$estado,$idconversacion);
...
$pdf->addConv($row_rsConversaciones['nombreremitente'],$destinatarios,$row_rsConversaciones['mensaje'],$estado[$row_rsConversaciones['estado']], $prioridad[$row_rsConversaciones['prioridad']],$row_rsConversaciones['fechacreado'],$row_rsConversaciones['asunto'],$row_rsConversaciones['idconversacion']);
