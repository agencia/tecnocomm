...
if ((isset($_POST['idcliente'])) && ($_POST['idcliente'] != "")) {
  $deleteSQL = sprintf("DELETE FROM cliente WHERE idcliente=%s",
  ...
  $Result1 = mysql_query($deleteSQL, $tecnocomm) or die(mysql_error());
   ...
   require_once('lib/eventos.php');
	$evt = new evento(29,$_SESSION['MM_Userid'],"Cliente eliminado: ".$_POST['idcliente']);
	$evt->registrar();
...
if (isset($_GET['idcliente'])) {
  $ide_RsCliente = $_GET['idcliente'];
}
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_RsCliente = sprintf("select * from cliente where idcliente=%s", GetSQLValueString($ide_RsCliente, "int"));
...
<table width="350" border="0" align="center" class="wrapper">
  ...
  <tr>
    <td>&nbsp;</td>
    <td>ESTAS SEGURO QUE DESEAS ELIMINAR AL CLIENTE:<span class="Estilo1"><?php echo $row_RsCliente['nombre']; ?></span>?</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
