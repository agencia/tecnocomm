...
$query_rsConversaciones = sprintf("SELECT DISTINCT c.*, u.nombrereal As nombreremitente FROM conversacion c JOIN conversacion_destinatario cd ON c.idconversacion = cd.idconversacion JOIN usuarios u ON u.id = c.remitente WHERE (c.remitente = %s OR cd.destinatario = %s) $bus $estado ORDER BY c.prioridad DESC,c.fechacreado ASC", GetSQLValueString($colname1_rsConversaciones, "int"),GetSQLValueString($colname2_rsConversaciones, "int"));
...
<select name="estado">
  ...
  <option  value="3" <?php if (!(strcmp(3, $_GET['estado'])) || !isset($_GET['estado'])) {echo "selected=\"selected\"";} ?>>Realizados y Nuevos</option>
</select>
</label>
<label>
