...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "nuevoAviso")) {
  $insertSQL = sprintf("INSERT INTO avisos (de, para, mensaje, fecha, hora, prioridad) VALUES (%s, %s, %s, now(), now(), %s)",
...
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_RsPara = "SELECT * FROM usuarios ORDER BY nombrereal ASC";
$RsPara = mysql_query($query_RsPara, $tecnocomm) or die(mysql_error());
$row_RsPara = mysql_fetch_assoc($RsPara);
$totalRows_RsPara = mysql_num_rows($RsPara);
...
<label>Para:<select name="para">
  <?php
do {  
?>
  <option value="<?php echo $row_RsPara['id']?>"><?php echo $row_RsPara['nombrereal']?></option>
  <?php
} while ($row_RsPara = mysql_fetch_assoc($RsPara));
  $rows = mysql_num_rows($RsPara);
  if($rows > 0) {
      mysql_data_seek($RsPara, 0);
	  $row_RsPara = mysql_fetch_assoc($RsPara);
  }
?>
</select>
