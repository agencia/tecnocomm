<?php require_once('Connections/tecnocomm.php'); ?><?php require_once('utils.php'); ?>
<?php
...
mysql_select_db($database_tecnocomm, $tecnocomm);
	$query_CoCli = sprintf("SELECT *,sub.nombre as nom,sub.estado as edo  FROM subcotizacion sub,cotizacion co ,cliente cli 
	  where sub.idcotizacion=co.idcotizacion and co.idcliente=cli.idcliente %s %s %s ORDER BY fecha desc ,identificador DESC",$cliente,$identificador,$estado);
...
<table width="100%" border="0" align="center">
  ...
  <tr>
    <td width="60">&nbsp;</td>
    <td width="110">&nbsp;</td>
    <td colspan="6" align="center" valign="middle"><a href="cotizaciones_nueva_cliente_ip.php" onclick="NewWindow(this.href,'Nueva Cotizacion','850','600','YES');return false"><strong><img src="images/bullet_16.jpg" alt="nueva" width="26" height="22" border="0" align="middle" />NUEVA COTIZACION</strong></a></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<?php if ($pageNum_CoCli < $totalPages_CoCli) { // Show if not last page ?>
  ...
  <?php if ($totalRows_CoCli > 0) { // Show if recordset not empty ?>
    <tr class="titleTabla">
      ...
      <td width="162" align="center" >CONSECUTIVO</td>
      <td width="103" align="center">FECHA</td>
      <td colspan="2" align="center">CLIENTE</td>
      <td width="300" align="center">IDENTIFICADOR</td>
  ...
  <?php do { ?>
	...
	this.style.backgroundColor = '#E2EBF4';" onmouseout="this.style.backgroundColor = '';">
      <td align="center" valign="top" width="71"><img src="images/state<?php echo $row_CoCli['edo'];?>.png" alt="editar" width="24" height="24"  border="0" title="<? echo $state[$row_CoCli['edo']];?>"/></td>
      <td align="right"><a href="#"  name="<?php echo $row_CoCli['idsubcotizacion']; ?>"><a href="cotizacion.detalle.ip.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&idcliente=<?php echo $row_CoCli['idcliente'];?>&tipo=0&idip=<?php echo $row_CoCli['idip']; ?>" onclick="if(confirm('Estas seguro que deseas modificar esta cotizacion?')){NewWindow(this.href,'Modificar Cotizacion','950','980','yes');return false}else{return false;}"><?php if ($totalRows_CoCli > 0) { if(($row_CoCli['edo']==1)){?><img src="images/Edit.png" alt="editar" width="24" height="24"  border="0" title="MODIFICAR COTIZACION"/><? }}?></a><a href="cotizacion.sub.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&type=1" onclick="if(confirm('Estas seguro que deseas crear una subcotizacion ?')){NewWindow(this.href,'Modificar Cotizacion','950','980','yes');return false}else{return false;}"><?php if ($totalRows_CoCli > 0) {if($row_CoCli['edo']==2){?><img src="images/reeditar.png" alt="editar" width="24" title="GENERAR SUBCOTIZACION" height="24"  border="0"/><? }}?></a><a href="cotizacion.sub.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&type=2" onclick="if(confirm('Estas seguro que deseas crear una conciliacion ?')){NewWindow(this.href,'Modificar Cotizacion','950','980','yes');return false}else{return false;}"><?php if ($totalRows_CoCli > 0) {if($row_CoCli['edo']==3){?><img src="images/consiliar.png" alt="editar" width="24" height="24"  border="0" title="Conciliar Cotizacion"/><? }}?>
...
<a href="cotizacion.detalle.ip.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&idcliente=<?php echo $row_CoCli['idcliente'];?>&tipo=0&idip=<?php echo $row_CoCli['idip']; ?>" onclick="if(confirm('Estas seguro que deseas modificar esta conciliacion?')){NewWindow(this.href,'Modificar conciliacion','950','980','yes');return false}else{return false;}"><?php if ($totalRows_CoCli > 0) { if(($row_CoCli['edo']==6)){?><img src="images/Edit.png" alt="editar" width="24" height="24"  border="0" title="MODIFICAR CONCILIACION"/><? }}?></a><a href="cotizacion.sub.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&type=3" onclick="if(confirm('Estas seguro que deseas crear una subconciliacion ?')){NewWindow(this.href,'Modificar Conciliacion','950','980','yes');return false}else{return false;}"><?php if ($totalRows_CoCli > 0) {if($row_CoCli['edo']==7){?><img src="images/reeditar.png" alt="editar" width="24" title="GENERAR SUBCONCILIACION" height="24"  border="0"/><? }}?></a>
        <a href="conciliacionEnviada.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>" onclick="NewWindow(this.href,'ENVIAR CONCILIACION','450','200','yes');return false"><?php if ($totalRows_CoCli > 0) {if(($row_CoCli['edo']==6)or($row_CoCli['edo']==7)or($row_CoCli['edo']==9)){?><img src="images/state2.png" width="24" height="24" border="0" title="ENVIAR CONCILIACION" /></a><? }}?><a href="conciliacionAutorizada.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>&estado=<? echo $row_CoCli['edo']; ?>" onclick="NewWindow(this.href,'Autorizar Cotizacion','450','200','yes');return false"><?php if ($totalRows_CoCli > 0) {if($row_CoCli['edo']==7){?><img src="images/state3.png" width="24" height="24" border="0" title="AUTORIZAR CONCILIACION" /><? }}?></a><a href="conciliacionPagada.php?idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>" onclick="NewWindow(this.href,'PAGAR Cotizacion','450','200','yes');return false"><?php if ($totalRows_CoCli > 0) { if(($row_CoCli['edo']==8)){ if(in_array(13,$array_niveles)){?><img src="images/state5.png" width="24" height="24" border="0" title="PAGAR CONCILIACION" /><? }}}?></a>		</td>
      <td align="center" class="printingStyle"><?php echo $row_CoCli['identificador2']; ?></td>
      <td align="center" class="printingStyle"><?php echo formatDateShort($row_CoCli['fecha']); ?></td>
      ...
      <td align="center" class="printingStyle"><?php if($row_CoCli['idip'] != 0){?><a href="index.php?idip=<?php echo $row_CoCli['idip'];?>&mod=detalleip"><?php echo $row_CoCli['idip']; ?></a><?php }else{ ?><a href="ip.nueva.fromcot.php?idcotizacion=<?php echo $row_CoCli['idcotizacion'];?>&idsubcotizacion=<?php echo $row_CoCli['idsubcotizacion'];?>" class="popup">Crear Ip</a><?php } ?></td>
    </tr>
