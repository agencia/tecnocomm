...
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_rsDetalle = sprintf("SELECT *, sb.moneda AS monedacotizacion FROM subcotizacionarticulo sb, articulo a WHERE idsubcotizacion = %s AND a.idarticulo = sb.idarticulo ORDER BY idsubcotizacionarticulo ASC", GetSQLValueString($colname_rsDetalle, "int"));
...
$precio[] = 0;//money_format('%i', $p);
$importe2[] = 0;//money_format('%i',($row_rsDetalle['cantidad'] * $p));
$importe[] = 0;//money_format('%i',(($row_rsDetalle['cantidad'] * $p)));
$sub = 0;//$sub + money_format('%i',($row_rsDetalle['cantidad'] * $p));
...
$precio[] = 0;//money_format('%!n', $p);
$importe2[] = 0;//=money_format('%!n',($row_rsDetalle['cantidad'] * $p));
$importe[] = 0;//money_format('%!n',(($row_rsDetalle['cantidad'] * $p)));
$sub = 0;//$sub + money_format('%!n',($row_rsDetalle['cantidad'] * $p));
...
$precio[] =0;// money_format('%!n',$row_rsCotizacion['monto']*$man);
$importe2[] = 0;//money_format('%!n',$row_rsCotizacion['cantidad']*$row_rsCotizacion['monto']*$man);
$importe[] =  0;//money_format('%!n',$row_rsCotizacion['cantidad']*$row_rsCotizacion['monto']*$man);
