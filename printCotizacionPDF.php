...
$tipocotizacion = $row_rsCotizacio['tipo'];
$colname_rsDetalle = "-1";
if (isset($_GET['idsubcotizacion'])) {
...
$precio[] =  $p;
$importe2[] =$row_rsDetalle['cantidad'] * $p;
$importe[] =($row_rsDetalle['cantidad'] * $p);
$sub = $sub + $row_rsDetalle['cantidad'] * $p;
	...
	$palabras = explode(" ",utf8_decode(trim($row_rsDetalle['descri'])));
...
//suministro Global
if(($row_rsCotizacion['tipo']  == 1) || ($row_rsCotizacion['tipo']  == 3)){
$counter++;
$par++;
...
$importe2[] = $row_rsCotizacion['cantidad']*$row_rsCotizacion['monto']*$man;
...
$sub = $sub + $row_rsCotizacion['monto'];
	...
	$palabras = explode(" ",utf8_decode($row_rsCotizacion['descrimano']));
...
switch($paginas){
	...
	//detalle
		 ...
		 $subtotal = 0;
		  ...
		  $subtotal = round($subtotal + $importe2[$row-1],2);
...
$cant = ($row_rsCotizacion['descuento']/100)*$subtotal;
$pdf->setDescuento($row_rsCotizacion['descuento']."%",format_money($cant));
	$subtotal = round($subtotal - $cant,2);
}

	$iva =($row_rsCotizacion['iva'] * $subtotal)/100;
	
	
	$pdf->catidadLetra(num2letras(money_format('%!n',$subtotal + $iva),false,true,$row_rsCotizacion['moneda']));
	$piva = round($row_rsCotizacion['iva'] ,0)." % I.V.A";
//Pie



$pdf->setPie(format_money($subtotal),format_money($iva),format_money(($subtotal + $iva)),utf8_decode($row_rsFirma['nombrereal']),utf8_decode($row_rsFirma['puesto']),utf8_decode($row_rsFirma['email']),utf8_decode($row_rsCotizacion['formapago']),utf8_decode($moneda[$row_rsCotizacion['moneda']]),utf8_decode($row_rsCotizacion['vigencia']),utf8_decode($row_rsCotizacion['tipoentrega']),utf8_decode($row_rsCotizacion['garantia']),utf8_decode($row_rsCotizacion['notas']),$piva);
...
$pdf->setTitle($row_rsCotizacion['nombre']);
	...
	//detalle
		 $subtotal = $row = 0;
		  ...
		  $subtotal = round($subtotal + $importe2[$row],2);
