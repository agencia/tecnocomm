...
if (isset($_POST['idsubcotizacion'])) {
  $colname_rsCotizacion = $_POST['idsubcotizacion'];
}
mysql_select_db($database_tecnocomm, $tecnocomm);
$query_rsCotizacion = sprintf("SELECT sb.idsubcotizacion, sb.identificador2, sb.estado, cc.nombre,cc.correo FROM subcotizacion sb,contactoclientes cc WHERE idsubcotizacion = %s AND sb.contacto = cc.idcontacto", GetSQLValueString($colname_rsCotizacion, "int"));
...
try{
  ...
  if($row_rsCotizacion['estado'] >= 6) {
      $file ="http://localhost/printCotizacionPDFv2.php?idsubcotizacion=".$_POST['idsubcotizacion'];
	 }else{  
	  $file ="http://localhost/printCotizacionPDF.php?idsubcotizacion=".$_POST['idsubcotizacion'];
	 }
 
 $filename = $row_rsCotizacion['identificador2'].".pdf";
...
$recipients->addTo($row_rsCotizacion['correo']);
//Use addCc()
if($_POST['concopia']){
...
<table width="490" border="0" cellpadding="0" cellspacing="0" align="center" class="wrapper">
  ...
  <tr>
    ...
    <td valign="top"><?php echo $row_rsCotizacion['correo']; ?></td>
    <td></td>
  </tr>
  <tr>
    <td height="25"></td>
